  ...
  const variationMax = variation
    ? variation.stockQuantity !== null && variation.stockQuantity !== undefined
      ...
      : item.product.trackInventory
        ? Math.floor(
            (item.product.stockQuantity ?? Infinity) / variation.quantity,
          )
        : Infinity
    : Infinity;
      ...
      {pickerProduct && (
        ...
        >
          ...
          >
            ...
            {(pickerProduct.variations || []).filter((v) => v.isActive).length >
            0 ? (
...
function POSPageInner() {
  ...
  const [searchPickerProduct, setSearchPickerProduct] =
        ...
        const matchedVariation =
          product.barcode === code
            ...
            : (product.variations || []).find(
                (v: ProductVariation) => v.barcode === code,
              );
  ...
  const addToCart = (
    ...
    const startQty =
      presetQty ?? (isScalable ? product.minOrderQty || step : 1);
      ...
      if (existing) {
        if (isScalable) {
          ...
          const next = presetOnly
            ? parseFloat(
                (existing.quantity + (presetQty as number)).toFixed(10),
              )
            : parseFloat(
                (Math.round((existing.quantity + step) / step) * step).toFixed(
                  10,
                ),
              );
  ...
  const addWeighedToCart = (product: Product, weightKg: number) => {
    ...
    const maxStock = product.trackInventory
      ...
      : Infinity;
    const maxQty = product.maxOrderQty
      ? Math.min(product.maxOrderQty, maxStock)
      : maxStock;
    ...
    setCart((prev) => {
      ...
      if (existing) {
        ...
        if (next > maxQty) {
          toast(
            `Only ${maxQty} ${unit} available for ${product.name}`,
            "error",
          );
          return prev;
        }
    ...
    if (added) {
      toast(
        `Added ${rounded} ${unit} of ${product.name} from the scale`,
        "success",
      );
        ...
        p.filter(
          (i) =>
            !(
              ...
              (i.variationId ?? undefined) === variationId
            ),
        ),
      );
      return;
            ...
            >
              ...
              >
                ...
                {(searchPickerProduct.variations || []).filter(
                      ...
                      .map((v) => {
                        ...
                        const available = dedicated
                          ...
                          : searchPickerProduct.trackInventory
                            ? Math.floor(
                                (searchPickerProduct.stockQuantity ??
                                  Infinity) / v.quantity,
                              )
                            : Infinity;
                        const soldOut = available <= 0;
                  ...
                  <p className="text-xs text-red-500">
                    No preset weights are configured for this product. Contact
                    an admin to add some in Product Settings.
                  </p>
                ) : (
                  <div className="flex flex-wrap gap-2">
