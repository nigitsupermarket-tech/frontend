    ...
    <div className="p-6 space-y-6">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-amber-700">
              Open a session before processing sales
            </p>
          </div>
          <Link
            href="/admin/pos"
      ...
      {staff.hasOpenSession && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-4 flex items-center gap-3">
          ...
          <Link
            href="/admin/pos"
            className="ml-auto px-4 py-2 bg-green-600 text-white text-xs font-bold rounded-lg hover:bg-green-700"
          >
            Go to POS →
          </Link>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <StatCard
          title="Orders Today"
          value={String(staff.today.orders)}
          icon={ShoppingCart}
        ...
        <StatCard
          ...
          value={formatNumber(staff.allTime.orders)}
          icon={Package}
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          title="Online Orders (Month)"
          value={formatNumber(data.onlineOrders.thisMonth)}
          icon={ShoppingCart}
          color="bg-blue-50 text-blue-600"
          href="/admin/orders"
        />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        <StatCard
          ...
          sub={`${data.customers.newThisMonth} new this month`}
          icon={Users}
          color="bg-indigo-50 text-indigo-600"
        ...
        <StatCard
          ...
          value={String(data.promotions.active)}
          icon={Tag}
          color="bg-rose-50 text-rose-600"
              ...
              {data.staffSales.map((s: any) => (
                <tr key={s.id} className="hover:bg-gray-50">
                  ...
                  <td className="py-3 text-right font-medium">
                    {formatPrice(s.today.sales)}
                  </td>
                  <td className="py-3 text-right font-semibold">
                    {formatPrice(s.thisMonth.sales)}
                  </td>
                </tr>
              ))}
            </tbody>
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">
          {isManager ? "Store sales overview" : "Complete business overview"}
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          title="Customers"
          value={formatNumber(data.customers.total)}
          sub={`${data.customers.newThisMonth} new`}
          icon={Users}
          color="bg-indigo-50 text-indigo-600"
          href="/admin/customers"
        />
        <StatCard
          title="Products"
          value={formatNumber(data.inventory.total)}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5">
        <h2 className="text-sm font-semibold text-gray-900 mb-5">
          Revenue — Last 30 Days (Online + POS)
        </h2>
            ...
            <tbody className="divide-y divide-gray-50">
              {(data.staffSales || []).map((s: any) => (
                <tr key={s.id} className="hover:bg-gray-50">
                  ...
                  <td className="py-3">
                    <span
                      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${s.hasOpenSession ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}
                    >
                      <span
                        className={`w-1.5 h-1.5 rounded-full ${s.hasOpenSession ? "bg-green-500" : "bg-gray-400"}`}
                      />
          ...
          <div className="flex items-center gap-3 bg-orange-50 border border-orange-100 rounded-xl p-4">
            ...
            <div>
              <p className="text-sm font-semibold text-orange-900">
                {data.inventory.lowStock} low stock items
              </p>
              <Link
                href="/admin/inventory"
          ...
          <div className="flex items-center gap-3 bg-amber-50 border border-amber-100 rounded-xl p-4">
            ...
            <div>
              <p className="text-sm font-semibold text-amber-900">
                {data.promotions.active} active promotions
              </p>
              <Link
                href="/admin/promotions"
...
export default function DashboardPage() {
  ...
  if (dashData.viewType === "sales")
    return <SalesDashboard data={dashData} chart={chart} />;
  if (dashData.viewType === "manager")
    return <AdminDashboard data={dashData} chart={chart} isManager />;
