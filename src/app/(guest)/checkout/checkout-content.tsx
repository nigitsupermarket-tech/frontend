...
import {
  Loader2, CheckCircle, Package, Truck, CreditCard, FileText,
  Plus, MapPin, Store, Tag, X, ChevronRight, Scale,
} from "lucide-react";
import { z } from "zod";
...
interface Address {
  ...
  address: string; addressLine2?: string; city: string; state: string;
  lga?: string; country: string; postalCode?: string; label?: string; isDefault: boolean;
}

interface ShippingMethod {
  id: string; name: string; type: "TABLE_RATE" | "FLAT_RATE" | "STORE_PICKUP";
  cost: number; estimatedMinDays?: number; estimatedMaxDays?: number;
  storeAddress?: { name: string; address: string; city: string; phone: string; hours: string };
}

interface Discount {
  id: string; code: string; type: "PERCENTAGE" | "FIXED_AMOUNT" | "FREE_SHIPPING";
  value: number; minOrderAmount?: number; maxDiscount?: number; name: string;
}

const STEPS = ["Address", "Shipping", "Payment", "Review"];

function calculateTotalWeight(items: any[]): number {
  return items.reduce((t, item) => t + (item.product?.weight || 0.5) * item.quantity, 0);
}
...
function LGASelector({
  ...
  state: string; lga: string; onChange: (lga: string) => void;
}) {
  ...
  return (
    <div>
      <label className="block text-xs font-semibold text-gray-700 mb-1">
        ...
        <span className="ml-1 font-normal text-gray-500">— used for precise shipping cost</span>
      </label>
      <select
        value={lga}
...
function AddressForm({ onSave }: { onSave: (addr: Address) => void }) {
  const [form, setForm] = useState({
    ...
    address: "", addressLine2: "", city: "", state: "Rivers",
    lga: "", country: "Nigeria", postalCode: "",
  });
  ...
  const handleSave = () => {
    ...
    onSave({ ...form, id: Math.random().toString(36).slice(2), isDefault: false });
  };

  const inp = "w-full border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:border-green-500 rounded";
  ...
  return (
    <div className="space-y-3 border border-gray-200 p-4 rounded bg-gray-50">
      ...
      <input placeholder="Street address *" value={form.address} onChange={(e) => set("address", e.target.value)} className={inp} />
      <input placeholder="Apartment/unit (optional)" value={form.addressLine2} onChange={(e) => set("addressLine2", e.target.value)} className={inp} />
      <input placeholder="City / Town *" value={form.city} onChange={(e) => set("city", e.target.value)} className={inp} />

      {/* State dropdown from LGA data */}
      <div>
        <label className="block text-xs font-semibold text-gray-700 mb-1">State *</label>
        <select value={form.state} onChange={(e) => handleStateChange(e.target.value)} className={inp + " bg-white"}>
      ...
      <LGASelector state={form.state} lga={form.lga} onChange={(v) => set("lga", v)} />

      <input placeholder="Address label (e.g. Home, Office)" value={form.label} onChange={(e) => set("label", e.target.value)} className={inp} />
...
export default function CheckoutPageContent() {
  ...
  const discountAmount = useMemo(() => {
    ...
    if (appliedCoupon.type === "FIXED_AMOUNT") return Math.min(appliedCoupon.value, subtotal);
    return 0;
  ...
  const loadShippingMethods = async (state: string, lga?: string) => {
    ...
    try {
      const res = await apiPost<any>("/shipping/calculate", {
        ...
        weight: totalWeight,
        categoryIds: items.map((i: any) => i.product?.categoryId).filter(Boolean),
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      if (res.data.valid) { setAppliedCoupon(res.data.discount); toast("Coupon applied!", "success"); }
      else setCouponError(res.data.message || "Invalid coupon");
    } catch (err) { setCouponError(getApiError(err)); }
    finally { setCouponLoading(false); }
  };

  const onSubmit = async (data: CheckoutForm) => {
    if (!selectedMethodId) { toast("Please select a shipping method", "error"); return; }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <a href="/products" className="text-green-600 hover:underline font-medium">Continue Shopping</a>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="bg-white p-8 rounded-lg shadow text-center max-w-md w-full mx-4">
          ...
          <div className="flex gap-3 justify-center">
            <a href={`/orders/${orderId}`} className="px-5 py-2 bg-green-600 text-white font-semibold rounded hover:bg-green-700">Track Order</a>
            <a href="/products" className="px-5 py-2 border border-gray-300 text-gray-700 font-semibold rounded hover:bg-gray-50">Continue Shopping</a>
          </div>
        </div>
      </div>
    );
          ...
          {STEPS.map((s, i) => (
            <div key={s} className="flex items-center flex-1">
              <div className="flex items-center gap-2">
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${i < step ? "bg-green-600 text-white" : i === step ? "bg-gray-900 text-white" : "bg-gray-200 text-gray-500"}`}>
                  {i < step ? "✓" : i + 1}
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
              ...
              <div className="bg-white border border-gray-200 p-5 rounded">
                ...
                {user && addresses.length > 0 && !showAddressForm && (
                  <div className="space-y-3 mb-4">
                    {addresses.map((addr) => (
                      <label key={addr.id} className={`flex items-start gap-3 p-3 border rounded cursor-pointer transition-colors ${selectedAddressId === addr.id ? "border-green-500 bg-green-50" : "border-gray-200 hover:border-gray-300"}`}>
                        ...
                        <div>
                          ...
                          <div className="text-xs text-gray-500 mt-0.5">
                            {addr.address}, {addr.lga ? `${addr.lga}, ` : ""}{addr.city}, {addr.state}
                          </div>
                ...
                {user && !showAddressForm && (
                  <button onClick={() => setShowAddressForm(true)} className="flex items-center gap-2 text-sm text-green-600 hover:text-green-700 mt-2">
                    <Plus className="w-4 h-4" /> Add new address
                  </button>
                )}

                <button onClick={handleAddressNext} className="mt-5 w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3 rounded flex items-center justify-center gap-2">
              ...
              <div className="bg-white border border-gray-200 p-5 rounded">
                ...
                {selectedAddress && (
                  <div className="mb-3 flex items-start gap-2 text-xs text-gray-500">
                    ...
                    <span>
                      ...
                      {shippingZoneName && <span className="ml-1 text-gray-400">({shippingZoneName})</span>}
                    </span>
                  </div>
                )}
                ...
                {shippingLoading ? (
                  ...
                  <div className="py-6 text-center">
                    ...
                    <p className="text-gray-500 text-sm">No shipping options for your area.</p>
                    <p className="text-xs text-gray-400 mt-1">Please contact us to arrange delivery.</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {shippingMethods.map((method) => {
                      const isFree = method.cost === 0 || appliedCoupon?.type === "FREE_SHIPPING";
                      return (
                        <label key={method.id} className={`flex items-start gap-3 p-4 border rounded cursor-pointer transition-colors ${selectedMethodId === method.id ? "border-green-500 bg-green-50" : "border-gray-200 hover:border-gray-300"}`}>
                          <input type="radio" checked={selectedMethodId === method.id} onChange={() => setSelectedMethodId(method.id)} className="mt-1" />
                          <div className="flex-1">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                {method.type === "STORE_PICKUP" ? <Store className="w-4 h-4 text-amber-600" /> : <Truck className="w-4 h-4 text-green-600" />}
                                <span className="font-semibold text-sm text-gray-900">{method.name}</span>
                              </div>
                              <span className="text-sm font-bold">
                                {isFree ? <span className="text-green-600">FREE</span> : formatPrice(method.cost)}
                              </span>
                            </div>
                            {method.estimatedMinDays !== undefined && method.estimatedMinDays > 0 && (
                              <p className="text-xs text-gray-500 mt-0.5">
                                Estimated: {method.estimatedMinDays}–{method.estimatedMaxDays} business days
                              </p>
                            )}
                            {method.type === "STORE_PICKUP" && method.storeAddress && (
                              <div className="mt-2 text-xs text-gray-600 bg-amber-50 border border-amber-100 rounded p-2">
                                <p className="font-semibold">{method.storeAddress.name}</p>
                                <p>{method.storeAddress.address}, {method.storeAddress.city}</p>
                                <p>{method.storeAddress.hours}</p>
                                <p className="text-green-700 font-medium">📞 {method.storeAddress.phone}</p>
                              </div>
                            )}
                          </div>
                        </label>
                ...
                <div className="mt-5 p-4 border border-dashed border-gray-300 rounded bg-gray-50">
                  ...
                  {appliedCoupon ? (
                    <div className="flex items-center justify-between bg-green-100 border border-green-300 rounded px-3 py-2">
                      ...
                      <button onClick={() => { setAppliedCoupon(null); setCouponCode(""); }}><X className="w-4 h-4 text-green-700" /></button>
                    </div>
                  ) : (
                    <>
                ...
                <div className="flex gap-3 mt-5">
                  <button onClick={() => setStep(0)} className="flex-1 border border-gray-300 text-gray-700 font-semibold py-3 rounded hover:bg-gray-50">Back</button>
                  <button onClick={() => setStep(2)} disabled={!selectedMethodId}
            ...
            {step === 2 && (
              <div className="bg-white border border-gray-200 p-5 rounded">
                <h2 className="font-bold text-gray-900 text-lg mb-4 flex items-center gap-2">
                  <CreditCard className="w-5 h-5 text-green-600" /> Payment & Contact
                </h2>
                <form className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-semibold text-gray-700 mb-1">Full Name *</label>
                      <input {...register("customerName")} className={inp} placeholder="Your full name" />
                      {errors.customerName && <p className="text-xs text-red-500 mt-0.5">{errors.customerName.message}</p>}
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-gray-700 mb-1">Phone *</label>
                      <input {...register("customerPhone")} className={inp} placeholder="+234 801 234 5678" />
                      {errors.customerPhone && <p className="text-xs text-red-500 mt-0.5">{errors.customerPhone.message}</p>}
                    </div>
                    <div className="sm:col-span-2">
                      <label className="block text-xs font-semibold text-gray-700 mb-1">Email *</label>
                      <input {...register("customerEmail")} type="email" className={inp} placeholder="your@email.com" />
                      {errors.customerEmail && <p className="text-xs text-red-500 mt-0.5">{errors.customerEmail.message}</p>}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {[
                        { value: "PAYSTACK", label: "Pay Online (Card / Bank Transfer)", icon: "💳" },
                        { value: "BANK_TRANSFER", label: "Manual Bank Transfer", icon: "🏦" },
                        {
                          value: "CASH_ON_DELIVERY",
                          label: selectedMethod?.type === "STORE_PICKUP" ? "Pay at Pickup (Cash)" : "Cash on Delivery",
                          icon: selectedMethod?.type === "STORE_PICKUP" ? "💵" : "📦",
                        },
                      ].map(({ value, label, icon }) => (
                        <label key={value} className="flex items-center gap-3 p-3 border rounded cursor-pointer hover:border-green-400 transition-colors">
                  ...
                  <div>
                    ...
                    <textarea {...register("notes")} rows={3} className={inp + " resize-none"} placeholder="Any special instructions?" />
                  </div>

                  <div className="flex gap-3">
                    <button type="button" onClick={() => setStep(1)} className="flex-1 border border-gray-300 text-gray-700 font-semibold py-3 rounded hover:bg-gray-50">Back</button>
              ...
              <div className="bg-white border border-gray-200 p-5 rounded">
                    ...
                    <div key={item.id} className="flex items-center gap-3 py-3 border-b border-gray-100">
                      <div className="w-12 h-12 bg-gray-100 rounded overflow-hidden flex-shrink-0">
                        {item.product?.images?.[0] && (
                          <Image src={item.product.images[0]} alt={item.product?.name || ""} width={48} height={48} className="w-full h-full object-contain" />
                        )}
                      </div>
                      <div className="flex-1">
                ...
                {selectedAddress && (
                  <div className="mb-3 p-3 bg-gray-50 rounded text-sm">
                    ...
                    <p className="text-gray-600">{selectedAddress.address}{selectedAddress.lga ? `, ${selectedAddress.lga}` : ""}, {selectedAddress.city}, {selectedAddress.state}</p>
                    <p className="text-gray-500 text-xs">{selectedAddress.phone}</p>
                  </div>
                )}
                {selectedMethod && (
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                  <div className="flex gap-3">
                    <button type="button" onClick={() => setStep(2)} className="flex-1 border border-gray-300 text-gray-700 font-semibold py-3 rounded hover:bg-gray-50">Back</button>
                    ...
                    >
                      {submitting ? <Loader2 className="w-4 h-4 animate-spin" /> : <>Place Order — {formatPrice(total)}</>}
                    </button>
                  </div>
                </form>
              </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white border border-gray-200 p-5 rounded sticky top-4">
              <h3 className="font-bold text-gray-900 text-sm uppercase tracking-wide mb-4">Order Summary</h3>
              <div className="space-y-2 text-sm mb-4">
                ...
                {discountAmount > 0 && (
                  <div className="flex justify-between text-green-700">
                    <span>
                      Discount{" "}
                      {appliedCoupon && <span className="bg-green-100 text-green-700 text-xs px-1.5 py-0.5 rounded ml-1">{appliedCoupon.code}</span>}
                    </span>
              ...
              <div className="mt-3 text-xs text-gray-400 flex items-center gap-1">
                <Scale className="w-3 h-3" /> Cart weight: {totalWeight.toFixed(2)}kg
              </div>
              {selectedAddress?.lga && (
                <div className="mt-2 text-xs text-gray-400 flex items-center gap-1">
                  <MapPin className="w-3 h-3" /> {selectedAddress.lga}, {selectedAddress.state}
                </div>
              )}
            </div>
          </div>
