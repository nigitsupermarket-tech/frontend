...
import {
  ...
  LogIn, UserPlus, ShoppingCart,
} from "lucide-react";
import { useCart } from "@/hooks/useCart";
import { useAuthStore } from "@/store/authStore";
import { useCartStore } from "@/store/cartStore";
import { formatPrice, getProductImage } from "@/lib/utils";
import { LoadingSpinner, EmptyState } from "@/components/shared/loading-spinner";
    ...
    <div className="fixed inset-0 z-[60] bg-black/60 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
        ...
        <div className="px-6 pt-6 pb-4 text-center relative">
          <button onClick={onClose} className="absolute right-4 top-4 p-1.5 text-gray-400 hover:text-gray-600 rounded-lg hover:bg-gray-100">
            <X className="w-5 h-5" />
          </button>
...
export function CartDrawer() {
  const {
    items, subtotal, itemCount, isLoading, isOpen, isGuest,
    closeCart, updateQuantity, removeFromCart,
  } = useCart();
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          ...
          <button onClick={closeCart} className="p-2 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {isLoading ? (
            ...
            <EmptyState
              ...
              action={
                <a href="/products" className="px-5 py-2.5 bg-green-600 text-white text-sm font-semibold rounded-xl hover:bg-green-700 transition-colors">
                  Shop Now
                </a>
              }
            />
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between mt-2.5">
                      ...
                      <div className="flex items-center bg-white border border-gray-200 rounded-xl overflow-hidden">
                        <button
                          onClick={() => updateQuantity(isGuest ? item.productId : item.id, item.quantity - 1)}
